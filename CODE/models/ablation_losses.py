...
# outputs -> shape BATCH_SIZE x 100
# targets -> shape BATCH_SIZE x 100
#
class L2Loss():
    ...
    def __call__(self, outputs, targets):
        ...
        if self.reduction == 'mean':
            ...
            losses = torch.sum(losses)/batch_size/100
...
def BCE_L2_loss(net, net_old, criterion, images, labels, current_classes, starting_label, ending_label, distillation_weight=1, outputs_normalization='sigmoid', alpha=100):
    ...
    if starting_label == 0:
        ...
        loss = (bce_loss + (distillation_weight*dist_loss))/batch_size/100
        #print(loss.item())
